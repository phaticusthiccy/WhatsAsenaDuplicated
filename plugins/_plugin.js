...
Asena.addCommand({ pattern: 'install ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, usage: '{}install <raw_url>' }, (async (message, match) => {
  try {
    ...
    if (response.statusCode == 200) {
      let plugin_name = response.body.match(/addCommand\({.*pattern: ["'](.*)["'].*}/);
      ...
      let plugins = (await PluginDB.findAll()).map((plugin) => plugin.dataValues.name);
