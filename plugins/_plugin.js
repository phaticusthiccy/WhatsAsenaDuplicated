...
Serena.addCommand({pattern: 'install ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN, dontAddCommandList: true}, (async (message, match) => {
    ...
    if (response.statusCode == 200) {
        ...
        if (find.includes(plugin_name)) {
            ...
            if (!match[1].includes('phaticusthiccy') && DEG.level > 99) {
             ...
             if (!match[1].includes('sahilteamx') && DEG.level > 99) {
                await message.client.sendMessage(message.jid,LANG.limit + DEG.level + '_', MessageType.text)
                fs.unlinkSync('/root/WhatsAsenaDuplicated/plugins/' + plugin_name + '.js')
            }
            ...
            else if (!match[1].includes('sahilteamx') && DEG.level < 100) {
                await Db.installPlugin(url, plugin_name)
                await new Promise(r => setTimeout(r, 400))
                await message.client.sendMessage(message.jid, Lang.UNOFF, MessageType.text)
                await new Promise(r => setTimeout(r, 400))
...
Serena.addCommand({pattern: 'plugin$', fromMe: true, dontAddCommandList: true, desc: Lang.PLUGIN_DESC}, (async (message, match) => {
...
Serena.addCommand({pattern: 'remove(?: |$)(.*)', fromMe: true, dontAddCommandList: true, desc: Lang.REMOVE_DESC}, (async (message, match) => {
