...
re-coded or modified by sahilteamx
to bring new ai
*/
...
if (conf.LANG == 'EN') vtalk_dsc = 'Starts to Serena voice chat.', reply_eva = '*Reply to Any Voice Message!*'
...
Serena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('Serena') && conf.TALKING_Serena !== 'true') {        
        ...
        var finm = message.message.replace('Serena', '').replace(' ', '')   
...
Serena.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (conf.TALKING_Serena == 'true' && ((!message.jid.includes('-')) || (message.jid.includes('-') && 
...
Serena.addCommand({ pattern: 'vtalk$', desc: vtalk_dsc,dontAddCommandList: true, fromMe: wk }, (async (message, match) => {
...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'Tam fonksiyonel Serena özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
    already_on = 'Serena yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'Serena yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'Serena, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'Serena, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional Serena features. Turn your account into a ai chatbot!'
    already_on = 'Serena artificial intelligence is already fully functional.'
    already_off = 'Serena artificial intelligence is currently running semi-functional.'
    succ_on = 'Serena Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'Serena Set to Semi-Functional! Please wait a bit! ☑️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ Serena സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'Serena കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'Serena AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'Serena പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി Serena സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

Serena.addCommand({ pattern: 'Serena ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.Serena on / off' }, (async (message, match) => {
    var Serena_status = `${conf.TALKING_Serena}`
    if (match[1] == 'on') {
        if (Serena_status == 'true') {
